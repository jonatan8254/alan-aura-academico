...
- **Fase 3 (construcción) — CERRADA; la 4 en curso.** ~~No adelantar el diseño físico: es `ARQ-01`, y aunque el `CDR` ya cerró sigue siendo el siguiente hito. Tampoco código todavía.~~ **Agotada.** `ARQ-01` (`SD-50`) escribió el diseño físico y **cerró sus cuatro deudas** —CSRF, cierre de sesión, `PER-H4` y `H-09`—; la compuerta propia que se le auguraba **no se ejecutó**. La construcción produjo **14 handlers** sobre 13 recursos REST, 4 tablas DynamoDB y un bucket S3, **16 pantallas** en 17 archivos, **38 pruebas** y un contrato compartido de **68 tipos**; el sistema está en línea en `https://alan-aura-academico.vercel.app`. **Falta la entrega académica y la evaluación formal de `RC-08`.**
...
- **Capas declaradas no ejecutadas: ninguna queda.** ~~La capa de infraestructura no se ejecutó ni en secuencia (`E-1` de `DS-00`) ni en el modelo de clases (`E-1` de `MC-00`); el mecanismo va a `ARQ-01`, tras el CDR.~~ **`E-1` cerrada** por `ARQ-01` y su implantación con CDK real (`backend/infra/`). El render del `.svg` de `MC-01` se cerró en `SD-39`.
- **Pendientes: viven todos en `docs/00_gobernanza/ESTADO_PIPELINE.md §Pendientes declarados`**, con quién los cierra y qué bloquean. La tabla tiene **37 filas — 32 cerradas, 4 abiertas y 1 declarada sin acción inmediata**, contadas a máquina (`SD-53`). **Ninguna de las cuatro bloquea nada**: dos son propagaciones documentales, una la revisión de nivel 6 del usuario y una la sobrepromesa de `OBJ-6` en `VIS-01`. Tres deudas más las deja la construcción: **sin pruebas automatizadas en el servidor**, **sin catálogo de recursos de ayuda**, y **sin modo oscuro** —decisión cerrada, no deuda—. **Lección nueva de la Fase 3:** seis defectos aparecieron solo al ejecutar y ninguno era detectable leyendo artefactos; un diseño sin ejecución es una hipótesis (`INF-01 §9.5`).
