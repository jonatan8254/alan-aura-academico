...
- **No adelantar** el **diseño físico** —claves de DynamoDB, tabla de *endpoints*, inventario de S3, IAM—: es `ARQ-01` y va **después del CDR**. Tampoco código. **Siguiente hito: el CDR** (`iconix-cdr-review`, hito 3), cuyo paquete de entrada está preparado en `MC-00 §10`.
- **Decisión ya tomada y registrada, no una instrucción pendiente:** la **capa de infraestructura no se ejecutó** ni en secuencia (`E-1` de `DS-00`) ni en el modelo de clases (`E-1` de `MC-00`): sin participantes `INF_`, sin repositorios, sin DAO, sin sesión como clase. El mecanismo va a `ARQ-01`, **tras el CDR**. En `MC-01` hay una segunda capa declarada no ejecutada: el **render del `.svg`**, que no se pudo generar por no haber PlantUML en el entorno — se declara sin afirmar que se revisó (`MC-00 §4.2`).
- **Pendientes: viven todos en `docs/00_gobernanza/ESTADO_PIPELINE.md §Pendientes declarados`**, con quién los cierra y qué bloquean. **`PER-H5` está cerrado** (`ADR-003`, SD-33): el MVP **no respalda el almacén de datos personales**, así que no hay copia que escape al borrado en cascada. El que más pesa ahora es **`PER-H2`**: la ventana de «+30 días» impide que **`RF-24` se cumpla de forma inmediata**, y se cierra junto con `V6-b`.
