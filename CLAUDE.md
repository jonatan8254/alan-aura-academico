...
- **Fase 2 (ICONIX) — avanzada:** producidos el **modelo de dominio** (`MD-01`), el **diagrama de casos de uso** (`DCU-01`), la **especificación textual** (`ECU-00…ECU-14`), el **análisis de robustez** (`DR-00…DR-14`) y la compuerta **`RPD-01`**.
...
- **Fuera de alcance por ahora (no adelantar):** **el diseño físico** —claves de DynamoDB, tabla de *endpoints*, inventario de S3, IAM— es `ARQ-01` y va **después del diagrama de clases y de su CDR**; adelantarlo garantiza retrabajo. Tampoco se escribe código todavía. **Siguiente artefacto ICONIX: los diagramas de secuencia (`DS-01…DS-14`)**, y en ellos la **capa de infraestructura de la skill no se ejecuta**: se declara como no ejecutada y los participantes se derivan solo de los `DR-XX`.
