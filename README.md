...
**Qué queda abierto.** **Nueve pendientes abiertos**, todos declarados, en [`ESTADO_PIPELINE.md §Pendientes declarados`](docs/00_gobernanza/ESTADO_PIPELINE.md) — con quién cierra cada uno y qué bloquea. El que más pesa es **`PER-H5`**: el respaldo de la base en S3 **escapa al borrado en cascada**, así que **`RF-24` no se cumple de extremo a extremo**. Se cierra en `ARQ-01`, y su plazo no es de calendario sino material: **antes de que haya personas reales**. El resto no rompe nada vigente.
