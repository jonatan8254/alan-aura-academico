...
└── docs/
    ...
    ├── 07_casos_uso/                 DCU-01 · especificaciones/ ECU-00…ECU-14 · robustez/ DR-00…DR-14 · secuencia/ DS-00…DS-14 + DOP-01 + TRZ-DS-01 + pruebas/ CP-00…CP-14
    ├── 08_diseno/                    DIS-00 (inventario + plan) · DIS-01 (sistema de diseño) + 16 mockups de alta fidelidad
    ├── 09_informe/                   INF-01 (informe académico, Markdown versionado + HTML/PDF derivados)
    └── 10_arquitectura/              ARQ-01 (diseño físico) · CONTRATO_API_v1 (contrato de la interfaz)

backend/                             14 handlers Lambda · infra/ (CDK: 4 tablas DynamoDB + bucket S3) · config/prompts/
frontend/                            16 pantallas · 17 componentes · 38 pruebas · Vite + React 19 + TypeScript
packages/contrato-api/               68 tipos compartidos entre cliente y servidor — la fuente del contrato
vercel.json                          configuración de despliegue del cliente
```
...
Paquete documental completo, **repositorio independiente** (SD-18), y **el MVP construido y en línea** en **[`https://alan-aura-academico.vercel.app`](https://alan-aura-academico.vercel.app)**. En la Fase 2 se produjeron el **modelo de dominio** ([`MD-01 v1.6`](docs/06_dominio/MD-01_modelo_dominio.puml)), el **diagrama de casos de uso** ([`DCU-01 v2.2`](docs/07_casos_uso/DCU-01_casos_uso.puml)), la **especificación textual de los 14 casos de uso** ([`ECU-00…ECU-14`](docs/07_casos_uso/especificaciones/)), el **análisis de robustez** ([`DR-00…DR-14`](docs/07_casos_uso/robustez/)) y la **base de diseño de interfaz** ([`DIS-00`/`DIS-01`](docs/08_diseno/) + 16 mockups de alta fidelidad en claro y oscuro).

La fase 2 pasó por una **primera pasada completa de correcciones** ([`PDR-01`](docs/00_gobernanza/PDR-01_primera_pasada_correcciones.md)), motivada por la retroalimentación docente ([`RET-01`](docs/00_gobernanza/RET-01_retroalimentacion_docente.md)) y por los hallazgos del propio análisis de robustez. Estado medido al cierre: los **cinco validadores en 0 errores**, las especificaciones también sin advertencias, **76/76 flujos** con criterio de aceptación asociado, **26/26 RF** con caso de uso propio y único, y las **16 clases** del dominio manifestadas en casos de uso y en robustez. Existe un **informe académico** ([`docs/09_informe/`](docs/09_informe/)) para la entrega, pendiente de actualizar a este estado. **Producidos también los 14 diagramas de secuencia** ([`DS-00…DS-14`](docs/07_casos_uso/secuencia/), **283 mensajes** en 0 errores), su **delta de 193 operaciones** ([`DOP-01`](docs/07_casos_uso/secuencia/DOP-01_delta_operaciones.md)), **181 casos de prueba** derivados de los 150 controladores, y la matriz [`TRZ-DS-01`](docs/07_casos_uso/secuencia/TRZ-DS-01_matriz_trazabilidad.md). **Y el modelo de clases de diseño** ([`MC-00`/`MC-01`/`COD-01`](docs/07_casos_uso/clases/)): **43 clases**, **201 operaciones**, 51 atributos, 11 enumerados y 80 relaciones. **La compuerta final ya se cruzó:** el [`CDR-01`](docs/07_casos_uso/CDR-01_revision_critica_diseno.md) se ejecutó, su retrabajo se aplicó (`SD-39`) y una **verificación independiente** lo contrastó (`SD-40`). **Veredicto determinado por el líder el 2026-08-05: `Aceptado con verificación de retrabajo`**, con **cero Críticos y cero Mayores**, tras **cinco verificaciones independientes** (`SD-40`…`SD-47`). El diseño físico (`ARQ-01`) se escribió después y está implementado.

**Arquitectura técnica (2026-08-01, [`ADR-002`](docs/01_vision/ADR-002_reversion_stack_serverless.md)):** la construcción usará **React 19 + Vite + TypeScript** con **Tailwind CSS v4 y shadcn/ui**, desplegado en **Vercel**, y un backend **sin servidor** en AWS — **Lambda** (Node 22) tras **API Gateway**, **DynamoDB** como base operativa y **S3 versionado** para configuración, activos y respaldos—, con **autenticación propia** y **Groq `gpt-oss-20b`** sin cambios como motor conversacional. Supera a `ADR-001-D1/D2/D5` (Django · SQLite · PythonAnywhere), y **ninguna condición de reversa declarada lo disparó**: fue decisión del equipo. La fase 2 de ICONIX **sobrevivió al cambio casi intacta**: los **16** diagramas y las 14 especificaciones no requirieron ningún ajuste, y solo se neutralizaron **dos líneas de prosa** en los anexos de `MD-01` y `DCU-01` — es el rendimiento de haber mantenido el dominio libre de tecnología. Quedan dos hallazgos declarados: `RA-01` de `ECU-03` puede reabrirse, porque cerrar sesión con cookie `httpOnly` exige respuesta del servidor; y **`PER-H5`**, de canon — el respaldo de la base de datos vive en S3 y **escapa al borrado en cascada**, por lo que «eliminar cuenta» no borra el respaldo hasta que se cierre en `ARQ-01`. El **diseño físico** (claves, *endpoints*, inventario de S3) queda para `ARQ-01`, **posterior al diagrama de clases y su CDR**.

**Qué queda abierto.** **Cuatro pendientes**, todos declarados, en [`ESTADO_PIPELINE.md §Pendientes declarados`](docs/00_gobernanza/ESTADO_PIPELINE.md) — con quién cierra cada uno y qué bloquea. **Ninguno bloquea nada**: dos son propagaciones documentales, uno la revisión de nivel 6 que sigue abierta desde la Fase 1, y uno una sobrepromesa viva en `VIS-01`. Para terminar faltan la **entrega académica** y la **evaluación formal de `RC-08`**, la rúbrica de coherencia de personaje con muestra de al menos diez diálogos. La construcción deja además tres deudas declaradas: el servidor no tiene pruebas automatizadas, el catálogo de recursos de ayuda está vacío y el cliente no implementa el modo oscuro. ~~`PER-H5` hacía que `RF-24` no se cumpliera de extremo a extremo~~ — **cerrado en `SD-33` por `ADR-003`**: el MVP no respalda el almacén de datos personales, así que no hay respaldo que escape al borrado.
