...
La **duración estimada** del desarrollo es de aproximadamente un mes (unos veinte días laborables), reservando los últimos dos o tres días para integración, contingencias y ensayo de la demostración. Las **ideas clave** que sustentan el proyecto son el acompañamiento no clínico, el principio de *seguridad emocional por encima del engagement*, la minimización de datos (el modelo de lenguaje recibe únicamente una cápsula mínima, nunca el historial en bruto) y la no persistencia del contenido conversacional. En cuanto a **tecnologías**, la interfaz se construirá con **React y TypeScript** —empaquetada con Vite y apoyada en **Tailwind CSS** y **shadcn/ui** para materializar el sistema de diseño propio— y se desplegará en **Vercel**; el backend será **sin servidor**, con funciones **AWS Lambda en Node y TypeScript** expuestas mediante **API Gateway**, **DynamoDB** como base de datos operativa y **S3** como almacén versionado de configuración, activos y respaldos; el generador conversacional sigue siendo la **API de Groq** con el modelo de pesos abiertos `openai/gpt-oss-20b`, y el control de versiones e integración continua se llevará en **GitHub con GitHub Actions**. El **alojamiento y el motor de persistencia** pueden reemplazarse por configuración. Con el **proveedor del modelo** el matiz es más fino y conviene no exagerarlo: lo agnóstico es el **contrato de gobierno** —cápsula, guardas y *gate*, que no cambian—, pero el nombre del proveedor sí aparece como actor en `DCU-01` y como frontera en `DR-06`, así que sustituirlo exige reetiquetar esos artefactos, no solo cambiar configuración (`ADR-002-D8`).
...
**ID:** MV-01 · **Familia:** MV (único) · **Hogar:** `docs/02_modelos_verbales/` · **Fecha:** 2026-08-04 · **Versión:** externa v2.7 / interna v2.7.0 (SD-32: cuatro correcciones de propagación que destapó el modelo de clases — ver «Cambio v2.7») · **Autoría:** Equipo Alan & Aura Académico — Jonatan E. Sánchez Vargas (líder/redacción), Santiago Bedoya García, Luis Fernando Montoya Rodríguez, Santiago Eusse Gil.
...
**Cambio v2.5 (SD-26):** **resolución de PER-H1 — la cápsula siempre existe tras el onboarding.** Se precisa **RN-01.4** (que decía «ningún campo de perfil es obligatorio» sin distinguir) y se añade **RN-01.6**: `character` no es un autorreporte de perfil sino la elección de interlocutor, precondición funcional de la conversación. Consecuencia: la `CapsulaDePerfil` **siempre existe** al terminar el onboarding, con `character` como contenido mínimo; los 4 autorreportes siguen siendo opcionales y sin *defaults*. No cambia lo que recibe el LLM (RN-01.3 intacta), ni clases, ni relaciones. Ver `PER-01` §8.
**Cambio v2.7 (SD-32):** **cuatro correcciones que destapó el modelo de clases de diseño** (`MC-00 §6`), todas de propagación pendiente y ninguna de contenido nuevo. (a) **`H-G`, defecto vivo:** la Parte A §4 decía todavía «**mil quinientos** caracteres por mensaje» contra `H-01` = **2.500**. Estaba escrito **en letras**, que es por qué ningún barrido de «1.500» —ni `verificar_coherencia.py`— lo encontró en dos pasadas anteriores. (b) **`H-F`:** §13.3 seguía con `Usuario–CapsulaDePerfil (1–0..1)`; `SD-26` y `PER-01 §11` la fijaron en **1–1 tras el onboarding** y esa resolución nunca llegó aquí. (c) **`H-F2`:** entran a §13.1 los tres dominios de valor que existían declarados en otros artefactos pero no en la sección que el proyecto les reserva — `Consentimiento.capa`, `TitularDeCuenta.rol` y `Usuario.estado` (derivado). (d) **`H-J`:** se declara la convención de ortografía —identificador `contrasena` sin eñe porque llega a código, «contraseña» en prosa—, que hasta ahora divergía entre `MV-01`/`PER-01` y los `.puml`. **Sin cambios en clases, relaciones ni en lo que recibe el LLM.**

**Cambio v2.6 (SD-29):** **reversión del stack a arquitectura sin servidor** (`ADR-002`). En la Parte A se reescribe la frase de **tecnologías** (React + Vite + TypeScript sobre Vercel; Lambda + API Gateway en Node/TypeScript; DynamoDB; S3 para configuración y respaldos; Groq se mantiene) y la de **disponibilidad** (capas gratuitas de Vercel y AWS en lugar de PythonAnywhere/Render). En §Candidatos y §14 las **exclusiones se neutralizan**: dejan de nombrar productos concretos y pasan a excluir «el framework de aplicación, el motor de persistencia y la plataforma de despliegue, cualesquiera que sean» — la regla siempre fue que la infraestructura no es dominio, y así no vuelve a caducar. **Sin cambios en reglas de negocio, clases, relaciones ni en lo que recibe el LLM.**
...
| LLM/Groq, el framework de aplicación, el motor de persistencia y la plataforma de despliegue —cualesquiera que sean—, gate, fallback, onboarding-como-proceso, chat-como-UI, mensajes de éxito/error | **Término UI/técnico → descartado** | No entran al dominio (§14 exclusiones). |
...
| RN-01.4 | Ningún **autorreporte** de la caracterización es obligatorio (`mood_self_report`, `energy_self_report`, `conversation_goal`, `response_style`): el usuario puede omitirlos todos. Obligatorios son solo la edad, el consentimiento y `character` (RN-01.6). | Habilitador | canon (minimización) |
| RN-01.5 | El consentimiento se puede revocar desde el onboarding y después. | Habilitador | RN-07 |
| RN-01.6 | `character` **no es un autorreporte de perfil** sino la elección de interlocutor (Alan o Aura): es **precondición funcional** de la conversación, del mismo rango que el consentimiento. En consecuencia, la `CapsulaDePerfil` **siempre existe** al terminar el onboarding, con `character` como contenido mínimo; omitir la caracterización deja la cápsula con solo ese campo, **sin *defaults*** para los omitidos. El valor persistido es la **última elección** y actúa como predeterminado: el personaje es cambiable por sesión (RN-02.6). | Restricción | RN-01.3, RN-02.6, plan §3.4; resolución de PER-H1 (SD-26) |
...
| RN-03.2 | El directorio muestra solo campos mínimos: alias, **ID truncado**, fecha de registro, **estado ∈ {activo, sin consentimiento vigente}** (derivado del `Consentimiento`, no editable ni suspensión) y onboarding completado. | Restricción | plan §3.7; SD-26 |
...
`Usuario.{username, alias, contrasena, edad (≥18), esAdulto (bool), versionDisclosure}` *(el identificador va sin eñe, como en `MD-01 §6` y los `.puml`, porque llega a código; en prosa se escribe «contraseña» — SD-32, `H-J`)* · **`CapsulaDePerfil` = `ContextoInicialConversacionalV1` (plan §3.4):** `mood_self_report ∈ {muy mal, mal, neutral, bien, muy bien, prefiero no responder}` (opcional) · `energy_self_report ∈ {baja, media, alta, prefiero no responder}` (opcional) · `conversation_goal ∈ {sentirme escuchado, calmarme, ordenar ideas, dar un paso pequeño, recibir una sugerencia breve, prefiero no responder}` (opcional) · `response_style ∈ {breve y directo, equilibrado, pausado y reflexivo, sin preferencia}` (opcional) · `character ∈ {alan, aura}` (**obligatorio**, `source: user_choice`) · metadatos `schema_version`, `consent_version`. Cada autorreporte lleva `{value, source: "self_report", collected_at}`. · `Consentimiento.estado ∈ {otorgado, revocado}` · `Conversacion.estado ∈ {activa, cerrada}` · `DisponibilidadDelChatbot.estado ∈ {habilitado, deshabilitado}` · `Consentimiento.capa ∈ {base, personalizacion}` (`ECU-12 §4.1`/§7, añadido en SD-32) · `TitularDeCuenta.rol ∈ {usuario, administrador}` (`PER-01 §3.1`, `RNF-08`) · `Usuario.estado ∈ {activo, sin consentimiento vigente}` (**derivado** del `Consentimiento`, no almacenado — `RN-03.2`, `PER-H3` cerrado en SD-26). Recursos/textos/parámetros del gate = **configuración por entorno** (no atributos de una clase de dominio).
...
Usuario–Consentimiento (1–1) · Usuario–CapsulaDePerfil (**1–1 tras el onboarding**; 0 antes — `PER-01 §11`, SD-26) · Usuario–Conversacion (1–0..*) · Conversacion–Mensaje (1–1..*) · Conversacion–Personaje (0..*–1) · Mensaje–EventoDeSeguridad (1–0..1) · EventoDeSeguridad–RecursoDeAyuda (1–1..*) · Administrador–DisponibilidadDelChatbot (1–1) · DisponibilidadDelChatbot–Conversacion (1–0..*).
...
**Exclusiones (no son clases de dominio):** LLM/Groq, y el **framework de aplicación, el motor de persistencia y la plataforma de despliegue, cualesquiera que sean** (la regla es que la infraestructura no es dominio, no que lo sea un producto concreto; el stack vigente está en `ADR-002`); módulo, pantalla, formulario, API, servicio, controlador, base de datos; `gate` y `fallback` (mecanismos); onboarding/chat como procesos o UI; registro/login/eliminar-cuenta como **acciones** (van a casos de uso, no a clases); mensajes de éxito/error; multiplicidades y atributos (reserva §13).
