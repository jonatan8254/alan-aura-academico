...
| P-13 | Gestión de cuenta y datos | [`mockups/p13_gestion_cuenta_datos.html`](mockups/p13_gestion_cuenta_datos.html) | `/cuenta/…` | CU-04, CU-11 y CU-12 · RF-24, RF-22, RF-23 | Usuario | Media | default · confirmar eliminación (destructivo) |
...
| P-09 | Onboarding · elegir Alan o Aura | [`mockups/p09_onboarding_elegir_personaje.html`](mockups/p09_onboarding_elegir_personaje.html) | `/onboarding/` | CU-14 · RF-06 | Usuario | Alta | comparar · seleccionado |
| P-10 | Chat con el acompañante (normal) | [`mockups/p10_chat_claro.html`](mockups/p10_chat_claro.html) · [oscuro](mockups/p10_chat_oscuro.html) | `/api/chat/` | CU-06 y CU-13 · RF-07…RF-10, RF-13, RF-25; RF-12 | Usuario · LLM | Alta | vacío/bienvenida · escribiendo · límite de sesión · cambiar personaje |
...
- **P-09 Elegir personaje** — Dos tarjetas (Aura teal / Alan ámbar) con avatar, rol y frase de muestra de su tono; comparar sin fricción. Es la interfaz de **CU-14 «Elegir acompañante»**, incluido por CU-05; dejó de pertenecer a CU-05 en el PDR-01. (E1, RF-06)
...
- **P-11 Errores del chat** — Estados claros para 401/403/400/409/429/502/504; mensaje humano + reintento; **sin** *stack* ni códigos crudos. (RF-26)

  > **Corrección del PDR-01 (hallazgo D-13).** Esta ficha decía «…y límite», duplicando el **límite de sesión** que §2 ya asigna a los estados de **P-10**. Se resuelve a favor de §2, que es la tabla de inventario y la fuente autoritativa: el límite de sesión es un estado de la conversación normal (`FA-01` de CU-06, que finaliza de forma controlada **sin error crudo**), no un estado de error. Los diagramas de robustez ya seguían §2.
...
- **P-13 Gestión de cuenta** — Pantalla **compartida por tres casos de uso**: reiniciar la caracterización (CU-11) · revocar la personalización (CU-12) · **eliminar cuenta** (CU-04). **Dos de las tres acciones son irreversibles**, no una: eliminar la cuenta borra en cascada, y **reiniciar la caracterización borra la cápsula entera, `character` incluido**, lo que deja al Usuario sin poder conversar hasta rehacer el onboarding. Solo **revocar la personalización es reversible**: la capa puede otorgarse de nuevo. Las dos irreversibles exigen confirmación explícita con su advertencia. (RF-22/23/24, PRIV-R11)

  > **Corrección del PDR-01 (hallazgo D-02).** Hasta la fase D.3 esta ficha decía «todo claro y reversible salvo eliminar», lo cual era **falso**: reiniciar la caracterización es igual de irreversible. El requisito de confirmación existía solo para eliminar la cuenta; ahora `ECU-11` lo exige también para reiniciar.
