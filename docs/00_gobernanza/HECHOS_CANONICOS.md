...
**ID:** HC-01 · **Familia:** gobernanza · **Hogar:** `docs/00_gobernanza/` · **Fecha:** 2026-08-04 · **Versión:** v2.0 (**`SD-53`: entran las cifras de la Fase 3 construida** — `H-32` a `H-35`: 16 pantallas, 38 pruebas, 14 handlers y 68 tipos del contrato compartido. Se **precisan** dos que se habían quedado cortas: `H-31`, porque 13 recursos REST se sirven con 14 métodos HTTP, y `H-03`, porque «tokens de salida» resultaron ser dos magnitudes distintas —respuesta visible y presupuesto de generación— y confundirlas produjo un error real. Se corrige el recuento de §Por qué existe, que decía «ocho de los veinticuatro» con la tabla en 35 filas). v1.9 (**`SD-50`:** entran `H-30` y `H-31`, las cifras de `ARQ-01` — 4 tablas DynamoDB y 13 *endpoints*). v1.8 (**`SD-48`, tras el cierre del `CDR`**: las cifras canónicas se **reverificaron** en la quinta verificación independiente y **ninguna se movió** —283, 262 = 15/38/150/59, 43/201/51/80, 193 y 181—; se cierra el **pendiente 1**, la declaración de los 1.500 caracteres, que estaba resuelto en los documentos y abierto solo en el registro). v1.7 (SD-40: la fila de valores obsoletos que `SD-39` **nunca escribió**, la nota que separa las 21 clases de la capa de secuencia de las 27 de `MC-01`, y `H-28` acotado a `MC-01`). v1.6 (SD-39: seis hechos se mueven por el retrabajo del `CDR-01`, entra `H-29` y los cuatro conteos de `MC-01` pasan a comprobarse por script). v1.5 (SD-34: el respaldo de la base de datos entra como valor obsoleto). v1.4 (SD-32: entran `H-25`…`H-28`, los conteos del modelo de clases de diseño, y dos filas nuevas de valor obsoleto). v1.3 (SD-31: se documenta el **cuarto bloque** del validador —`VERSIONES DECLARADAS`— y por qué su alcance es deliberadamente estrecho). v1.2: H-20 en **262**, H-21 en **150** y tres hechos nuevos del paquete de secuencia · **Estado:** vigente.
...
**Qué comprueba automáticamente `scripts/verificar_coherencia.py`, y qué no.** Comprueba **catorce** de los treinta y cinco hechos: H-01, H-04, H-05, H-10, H-22, H-23, H-25, H-26 y H-27 (que ningún valor obsoleto sobreviva como afirmación viva) y H-11, H-12, H-13, H-14, H-25, H-26, H-27, H-29 (contando directamente sobre los `.puml`; H-25 a H-27 caen en los dos grupos). Los demás —H-02, H-03, H-06 a H-09, H-15 a H-21, H-24, H-28 y H-30 a H-35— **son responsabilidad del lector**: están aquí para que exista una fuente única, no porque una máquina los vigile. *(El recuento decía «ocho de los veinticuatro» desde v1.2 y se quedó ahí mientras `SD-39` añadía cinco valores obsoletos y cuatro conteos, y `SD-50`/`SD-53` seis hechos nuevos. Corregido en `SD-53`.)* **Desde `SD-31` hay un cuarto bloque, `VERSIONES DECLARADAS`,** que contrasta el inventario de `INDICE_MAESTRO` contra la ficha de cada artefacto. Nació de un error real: el inventario decía `DS-00…14 v1.0` cuando `DS-00` iba por v1.3, y una lectura a ojo no lo vio. Su alcance es **deliberadamente estrecho**: una primera versión miraba toda mención `ARTEFACTO vX.Y` del corpus y dio **202 hallazgos, casi ninguno un defecto** —la inmensa mayoría son procedencia, «`Visitante` | Clase de `MD-01 v1.4`», que es legítimamente histórica—. Un bloque con esa relación señal/ruido no se lee: se ignora, y entonces no vigila nada. El inventario, en cambio, **sí afirma el estado actual**, y ahí una versión mal se propaga a quien lo consulte.
...
| H-03 | Tokens de la **respuesta visible** del LLM | **350**, aplicados por `limitarTokensDeSalida()` sobre el texto ya generado. Distinto del **presupuesto de generación** que se envía a Groq, hoy **2.000**: `gpt-oss-20b` razona antes de responder y ese razonamiento consume el mismo presupuesto, así que 350 como `max_tokens` agotaba la cuota pensando y devolvía la cadena vacía. `RN-02.8` se refiere a la respuesta visible y sigue cumpliéndose | `REQ-01` RF-25 | `MV-01` RN-02.8, `ECU-06` §15, `groq.ts`, `guardasDeSalida.ts` |
...
| H-06 | Intercambios de sesión enviados al LLM | **≤ 4**, que son **8 mensajes**: un intercambio es una ida y vuelta. El código lo capó a 4 *mensajes* y recortó la memoria a la mitad hasta que se corrigió el 2026-08-06 | `REQ-01` RNF-04 | `PRIV-R1`, `MV-01` RN-02.2/RN-03, `ECU-06` §17, `chat.ts` |
...
| H-28 | Clases del espacio de la solución **en `MC-01`** | **27** = 2 de control + 1 de auditoría + 18 de frontera + 6 tipos de transferencia | `MC-01_matriz_procedencia.md §4` | `MC-00 §3`, `COD-01`, `ESTADO_PIPELINE`. **No `DOP-01 §8` ni `DS-00`:** ver la nota de abajo |
...
| H-30 | Tablas DynamoDB de `ARQ-01` | **4** = 1 de diseño de tabla única (`Titular`, para las 4 entidades ligadas a la cuenta) + 3 separadas (`Configuracion`, `EventoOperativo`, `AccionAdministrativa`) | `ARQ-01-D2` | `REGISTRO_DECISIONES` (`SD-50`), `CHANGELOG`, `ESTADO_PIPELINE` |
| H-31 | *Endpoints* de la tabla de `ARQ-01` | **13** recursos REST bajo `/api/v1/`, servidos por **14** métodos HTTP: `/admin/chat-access` admite `POST` y `GET`. La segunda se añadió al implementar `P-16`, después de acuñarse la cifra 13 | `ARQ-01-D3` | `REGISTRO_DECISIONES` (`SD-50`), `CHANGELOG`, `CONTRATO_API_v1` |
| H-32 | Pantallas del frontend | **16**, implementadas en 17 archivos: `Chat.tsx` cubre `P-10`, `P-11` y `P-12` como tres estados de una pantalla | `DIS-00` | `DIS-01`, `frontend/README.md`, `INF-01` |
| H-33 | Pruebas automatizadas | **38** en 5 suites, **todas de frontend** — `backend/` no tiene ninguna | `frontend/src/**/*.test.tsx` | `frontend/README.md`, `INF-01` |
| H-34 | Handlers Lambda | **14**, uno por método HTTP de `H-31` | `backend/src/handlers/` | `ARQ-01-D3`, `backend/CONTINUAR_AQUI.md` |
| H-35 | Tipos del contrato compartido | **68** = 14 entidades + 11 enumerados + 43 de rutas | `packages/contrato-api/src/` | `CONTRATO_API_v1`, `INF-01` |

### Nota sobre `H-28`: 21 y 27 miden cosas distintas, y ninguna está sin propagar

Hasta `SD-32` los dos valores coincidían en **21**, así que la columna «también se afirma en» de `H-28` listaba `DOP-01 §8`, `TRZ-DS-01 §3` y `DS-00 §7` como si dijeran lo mismo. **`SD-39` rompió esa coincidencia** al añadir a `MC-01` los **6 tipos de transferencia** de `H-04`, y la columna no se actualizó — de modo que un barrido de propagación marcaría como defecto lo que es una medición correcta.

| Artefacto | Valor | Qué cuenta |
|---|---|---|
| `MC-01` y su matriz §4 | **27** | El modelo de clases completo, con los 6 tipos de transferencia |
| `DOP-01 §8`, `DS-00` | **21** | Lo que introdujo el **delta de secuencia**: 2 control + 1 auditoría + 18 frontera. **Ningún diagrama de secuencia contiene un tipo de transferencia** |

**Regla:** en un artefacto de la capa de secuencia, **21 es correcto**; en uno de la capa de clases, **27**. Detectado en el barrido de `SD-40`.
...
| 3 · **21** clases del espacio de la solución | **27** (H-28) | `CHANGELOG`, `REGISTRO_DECISIONES` (SD-30, SD-32), y los bloques de corrección de `DOP-01 v1.2`, `TRZ-DS-01 v1.1` y `DS-00 v1.4` que documentan el cambio | `DOP-01 §8`, `TRZ-DS-01 §3`, `DS-00 §7` como afirmación vigente |

| 282 mensajes · 192 operaciones del delta · 37 clases · 200 operaciones · 35 atributos · 73 y 71 relaciones | **283 · 193 · 43 · 201 · 51 · 80** (H-22, H-23, H-25, H-26, H-27, H-29) | `CHANGELOG`, `REGISTRO_DECISIONES`, los bloques de cambio de las fichas que documentan el retrabajo (`DS-00 v1.6`, `DOP-01 v1.3`, `MC-00 v1.8`…), los campos **Insumos** que citan la versión consumida (`DOP-01 v1.1` (192 operaciones)), y `CDR-01`, cuyo §1 describe el paquete tal como se recibió al cierre de `SD-32` | Cualquier total vigente, fila de «hechos canónicos», tabla de resumen o descripción del estado actual. **Esta fila faltaba: `SD-39` movió seis hechos sin declarar ninguno obsoleto**, contra la regla de §Mantenimiento — y por eso el barrido de `SD-40` encontró once puntos vivos que nadie vigilaba |
...
**1. ~~El pendiente de los 1.500 caracteres ya está resuelto en los documentos, pero sigue declarado como abierto.~~ CERRADO en `SD-48`:** la declaración falsa se retiró de `ECU-00 §7`, que era donde vivía.
...
|---|---|---|---|
| v2.0 | 2026-08-06 | Equipo Alan & Aura Académico | **`SD-53`.** Entran las cifras de la Fase 3 construida: `H-32` (16 pantallas en 17 archivos), `H-33` (38 pruebas, todas de frontend), `H-34` (14 handlers Lambda) y `H-35` (68 tipos del contrato compartido). Se precisan `H-31` —13 recursos REST servidos por 14 métodos HTTP, porque `/admin/chat-access` admite `POST` y `GET`— y `H-03`, que separaba mal la respuesta visible (350) del presupuesto de generación (2.000). Se corrige el recuento de hechos vigilados por máquina. |
| v1.9 | 2026-08-05 | S. Bedoya | **`SD-50`.** Entran dos hechos del diseño físico: `H-30` (4 tablas DynamoDB de `ARQ-01`: 1 de diseño de tabla única + 3 separadas) y `H-31` (13 *endpoints* bajo `/api/v1/`). Ninguna cifra anterior se mueve. |
| v1.8 | 2026-08-05 | J. Sánchez | **`SD-48` — cierre del expediente del `CDR`.** Las nueve cifras canónicas se reverificaron a lo largo de cinco verificaciones independientes y **ninguna se movió**: `SD-41`…`SD-47` tocaron 42 archivos sin alterar 283 mensajes, 262 elementos de robustez, 43/201/51/80 de `MC-01`, 193 operaciones distintas ni 181 casos de prueba. El trinquete lo comprueba en cada ejecución. **Pendiente 1 cerrado:** la afirmación de que «`MV-01` §7.3 y `RF-25` aún dicen 1.500» era falsa y se retiró de `ECU-00 §7`. |
| v1.7 | 2026-08-05 | J. Sánchez | **SD-40, tras la verificación independiente.** Entra **la fila de valores obsoletos que `SD-39` nunca escribió**: movió seis hechos y no declaró obsoleto ninguno, contra la regla de §Mantenimiento de esta misma tabla. Esa omisión es la causa directa de que el barrido de `SD-40` encontrara **once puntos vivos** con cifras caducas —cuatro de ellos invisibles para el `CDR-01 v1.4`, que solo revisó las ubicaciones que su hallazgo `VI-06` alcanzó a listar—. Entra además la **nota sobre `H-28`**, que resuelve una ambigüedad real y no un descuido: hasta `SD-32` las clases de solución eran **21** en todas partes, y `H-04` subió a **27** solo las de `MC-01` al añadir los seis tipos de transferencia. `DOP-01` y `DS-00` siguen en **21 con razón** —ningún diagrama de secuencia contiene un tipo de transferencia—, así que `H-28` se acota a `MC-01` y su columna de consumidores deja de listarlos. **Ninguna cifra se mueve en esta versión**; lo que cambia es qué se declara obsoleto y dónde cada valor es correcto. |
