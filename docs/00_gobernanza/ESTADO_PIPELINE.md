...
## Qué sigue
1. **Corregir los tres hallazgos que la verificación independiente dejó abiertos** (`CDR-01 v1.4`, `SD-40`): `VI-01`, `VI-02` —los dos **Mayores**— y `VI-03`. **La verificación ya se hizo** y salió mejor de lo previsto en un punto y peor en otro.
...
| 19 | **`VI-01` — los 17 `opt` caen al flujo de éxito** | **Mayor.** La sustitución `break`→`opt` de `H-01` es correcta en recuento pero **no implementa el desenlace del texto**: `opt` solo salta su cuerpo, así que cancelar un borrado continúa a la cascada y un *timeout* del proveedor continúa a «texto generado». Afecta a `DS-03/04/05/06/10/11/12` | `/uml-sequence-diagram` — rediseñar con fragmentos mutuamente excluyentes (`alt`/`loop`) y reinspeccionar las 17 contra su fila de `ECU` | **El paso al código** | `CDR-01 §8-quinquies` |
| 20 | **`VI-02` — la nueva semántica no se propagó por la costura** | **Mayor.** `ECU`/`DS`/`MC` cambiaron, pero `DR-04`/`DR-11`, `DOP-01:140`, `COD-01:78,128` y `CP-04`/`CP-05`/`CP-11` **siguen mandando «deshacer»** el borrado que `H-02` y `H-15` eliminaron | Las skills dueñas de cada capa; repetir la costura `ECU→DR→DS→MC→COD/CP` | **El paso al código** | `CDR-01 §8-quinquies` |
...
>
> **El recuento, contado sobre las filas y no estimado.** La tabla tiene **22** filas: **7 cerradas**
> (1, 2, 3, 12, 13, 14 y ahora **15**), **12 abiertas** (4–11, 19, 20, 21 y 22) y **3 declaradas sin acción
> inmediata** (16, 17, 18), que no cuentan como trabajo abierto. *(La nota anterior decía «once abiertos» y
...
>
> **Lo que bloquea el paso al código ya no es la falta de verificación, sino su resultado.** La fila 15 se
> cerró —la verificación independiente se hizo, y la hizo **otro modelo**—, pero abrió las filas **19, 20 y
> 21**. Dos de ellas son **Mayores** y tocan **comportamiento**: `VI-01`, que deja diecisiete ramas cayendo al
> flujo de éxito, y `VI-02`, que dejó media costura ICONIX mandando deshacer un borrado que ya no se deshace.
> **El veredicto sigue en `Reinspección requerida`, y lo determina el líder**, no la revisión.
