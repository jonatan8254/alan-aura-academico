...
| 22 | **`VI-07`, `VI-08` y `SD-40-H1` — tres huecos del instrumental** | Moderados, del **instrumento**, no del paquete. `VI-07`: los bloques 3 y 5 se apoyan en `git diff HEAD`, así que en una rama ya comiteada pasan en verde sin revisar nada. `VI-08`: `VALORES_OBSOLETOS` solo vigila cuatro hechos — **por eso nadie vio los once puntos que `SD-40` tuvo que corregir a mano**. **`SD-40-H1`, nuevo y hallado al probar las guardas:** la **segunda guarda de `generar_cabeceras_mc01.py` es inalcanzable**. Su regex de alias captura con `(\w+)`, que **corta en el primer carácter no-palabra**, así que `as B_Landing-P01` se lee como `B_Landing`; la guarda comprueba `IDENTIFICADOR.match()` sobre un alias **ya truncado**, que por construcción siempre es válido. No solo no dispara: **emite una clase con un identificador distinto del que el modelo declara**, en silencio. Las guardas 1 y 3 **sí disparan** (código 3, comprobado) | Decisión aparte: tocar el instrumento de verificación no es edición documental. Se suma al pendiente 16 | Nada hoy — ningún alias de `MC-01` es malformado, comprobado | `CDR-01 §8-quinquies`, `scripts/verificar_coherencia.py`, `docs/07_casos_uso/clases/scripts/generar_cabeceras_mc01.py:123-127` |
| 16 | **Punto ciego del bloque 4** (`H-24`) | `verificar_coherencia.py` **no puede ver** la versión de `TRZ-DS-01`: su regex de ID lee `TRZ-DS-01` como `DS-01` y atribuye la versión a otro artefacto, que además es un `.puml` sin ficha, así que la comparación **se salta en silencio**. Es el **único ID compuesto** del repositorio. El **dato** ya está corregido; **el script no** | Decisión aparte: tocar el instrumento de verificación no es una edición documental | Nada hoy — pero cualquier desfase futuro de `TRZ-DS-01` volvería a pasar inadvertido | `CDR-01 §8-quater` |
...
| 18 | **Nota de versión de `MD-01.md` y `DCU-01.md`** | Sus `.puml` ganaron `title` en `SD-39` y sus `.md` no lo registran. `verificar_coherencia.py` **no lo exige** —solo mira `.md` modificados— y la razón está escrita dentro de cada `.puml`, pero conviene decidirlo en vez de dejarlo tácito | El líder, en la próxima pasada de gobernanza | No bloquea | `SD-39` |
...
>
> **El recuento, contado sobre las filas y no estimado.** La tabla tiene **23** filas: **10 cerradas**
> (1, 2, 3, 12, 13, 14, 15, 19, 20 y ahora **21**), **10 abiertas** (4–11, 22 y la nueva **23**) y **3 declaradas
> sin acción inmediata** (16, 17, 18), que no cuentan como trabajo abierto. *(La nota de `SD-37` decía «once abiertos» y
