...
**Propósito:** decir en qué fase está el subproyecto y qué sigue. Es el primer archivo a leer al rehidratar contexto. **Fecha de creación:** 2026-07-12 · **Última actualización:** 2026-08-01 · **Versión:** v1.5 (SD-30: los 14 diagramas de secuencia `DS-01…DS-14`; robustez pasa a 263 elementos y 150 controladores por la corrección de `DR-01` y `DR-04`). v1.4 corrigió el conteo de `.puml` de la Fase 2: son **16**, no 17.

---

## Dónde estamos
Fase **1 cerrada** y **Fase 2 (ICONIX) avanzada**: producidos y verificados los cinco primeros artefactos — el **modelo de dominio `MD-01 v1.4`** (`docs/06_dominio/`, 16 clases), el **diagrama de casos de uso `DCU-01 v2.1`** (`docs/07_casos_uso/`, 14 casos de uso), la **especificación textual `ECU-00…ECU-14`** (`docs/07_casos_uso/especificaciones/`), el **análisis de robustez `DR-00…DR-14`** (`docs/07_casos_uso/robustez/`, 263 elementos) y la **compuerta ICONIX `RPD-01`** entre análisis y diseño detallado (`docs/07_casos_uso/RPD-01_revision_preliminar_diseno.md`).
...
| Robustez | 10 diagramas, 179 elementos | **14 diagramas, 263 elementos**, 0 errores |
...
## Qué sigue
1. **Diagramas de secuencia `DS-01…DS-14`** (siguiente artefacto ICONIX): la skill `uml-sequence-diagram` ya está disponible. Convierte cada controlador de robustez en un mensaje, asigna operaciones a las clases del dominio y produce el delta de operaciones + los casos de prueba que consume. Con **150 controladores** en los 14 diagramas de robustez, esa es la cota inferior de casos de prueba (`CP-XX`) que resultarán.
