...
| 19 | ~~**`VI-01` — los 17 `opt` caen al flujo de éxito**~~ | ✅ **CERRADO en SD-41.** Corregido rama por rama, releyendo cada fila de `ECU` en vez de aplicar un operador por categoría — que es el error que produjo `H-01` y después `VI-01`. **Trece de los diecisiete resultaron ser `break` legítimos** (su `ECU` dice «Termina»), y los otros cuatro pasaron a `alt` con el sufijo de éxito en la rama excluyente. **283 mensajes intactos** | — | Ya no bloquea | `CDR-01 §8-quinquies`, `DS-00 §E-5` |
...
>
> **El recuento, contado sobre las filas y no estimado.** La tabla tiene **23** filas: **13 cerradas**
> (1, 2, 3, 12, 13, 14, 15, 16, 18, 19, 20, 21 y **22**), **9 abiertas** (4–11 y la nueva **23**) y **1 declarada
> sin acción inmediata** (17, que es de la skill y no de este repositorio). Suman 23.
>
