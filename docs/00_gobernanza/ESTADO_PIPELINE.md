...
**Propósito:** decir en qué fase está el subproyecto y qué sigue. Es el primer archivo a leer al rehidratar contexto. **Fecha de creación:** 2026-07-12 · **Última actualización:** 2026-08-01 · **Versión:** v1.7 (SD-31: nueva sección **«Pendientes declarados»**, que reúne los once que estaban dispersos —`PER-H5` y `COD-01` entre ellos—; y los 14 SVG de robustez regenerados sin colisiones). v1.6: SD-30 completo, con los 14 diagramas de secuencia, sus 181 casos de prueba y los 7 hallazgos aplicados; robustez en **262 elementos**. v1.4 corrigió el conteo de `.puml` de la Fase 2: son **16**, no 17.

---

## Dónde estamos
Fase **1 cerrada** y **Fase 2 (ICONIX) avanzada**: producidos y verificados los cinco primeros artefactos — el **modelo de dominio `MD-01 v1.4`** (`docs/06_dominio/`, 16 clases), el **diagrama de casos de uso `DCU-01 v2.1`** (`docs/07_casos_uso/`, 14 casos de uso), la **especificación textual `ECU-00…ECU-14`** (`docs/07_casos_uso/especificaciones/`), el **análisis de robustez `DR-00…DR-14`** (`docs/07_casos_uso/robustez/`, 262 elementos) y la **compuerta ICONIX `RPD-01`** entre análisis y diseño detallado (`docs/07_casos_uso/RPD-01_revision_preliminar_diseno.md`).
...
## Qué sigue
1. **Diagrama de clases de diseño** (siguiente artefacto ICONIX): la skill `uml-design-class-model` consume directamente `DOP-01`, que ya trae las **192 operaciones** con su clase receptora y su justificación. Es el artefacto que fija **tipos y firmas** —y por eso `COD-01` se difirió hasta él: escribir firmas antes sería inventarlas.

   > **Después del diagrama de clases va el CDR** (`iconix-cdr-review`, hito 3) y **solo después `ARQ-01`**, el diseño físico: claves de DynamoDB, tabla de *endpoints*, inventario de S3, IAM y red. El motivo lo escribe `ADR-002 §1` sin rodeos: *«el diagrama de clases aún puede mover atributos y operaciones. Fijar claves antes de esa convergencia produce retrabajo garantizado.»* `ARQ-01` hereda además tres deudas: **CSRF**, el ***endpoint* de cierre de sesión** que reabre `RA-01` de `ECU-03`, y **`PER-H5`** —que debe cerrarse **antes de cualquier uso con personas reales**, porque mientras siga abierto `RF-24` no se cumple de extremo a extremo.
...
2. **Los once pendientes declarados** viven ahora en su propia tabla, más abajo: `PER-H5` (que rompe `RF-24`), `COD-01`, los dos `PER-H` de esquema, los dos `RA-01`, CSRF, los dos de `HECHOS_CANONICOS`, el informe académico y la revisión de nivel 6.
...
11. **Grafo de conocimiento propio del subproyecto (SD-27, cerrado 2026-07-25):** `grafo/` — 2.876 nodos, vault de Obsidian trazable. **No se ha regenerado** desde entonces: no incluye `MD-01 v1.4`, `DCU-01 v2.1`, las 14 ECU, los 14 DR, ni `PDR-01`/`RET-01`/`RPD-01`. Instructivo de uso: `GUIA_USO_GRAFO_Y_VAULT.md` (raíz).
...
que buscar es un pendiente que se olvida. Aquí están los diez, con quién los cierra y qué bloquean.
...
|---|---|---|---|---|---|
| 1 | **`PER-H5`** ⚠️ | El respaldo de la base en **S3 escapa al borrado en cascada**. `PER-T1` y `PRIV-R11` enumeran la cascada sin mencionarlo, porque se escribieron cuando no había respaldos; y el versionado agrava el punto, porque borrar un objeto no lo borra: sobreviven sus versiones | **`ARQ-01`**, y **antes de cualquier uso con personas reales** | **`RF-24` no se cumple de extremo a extremo.** Es el único RF en esa situación, y está marcado como tal en `TRZ-01 §2` | `ADR-002-D6`, `PER-01 §8` |
| 2 | **`COD-01`** | Los insumos para código (clase · operación · **firma** · capa). **No es un olvido, es una espera deliberada:** la columna de firma exige tipos, y los tipos los fija el diagrama de clases. Escribirlos ahora sería inventarlos, y además `iconix-cdr-review` los genera desde el modelo de clases, con lo que habría dos fuentes en conflicto | El **diagrama de clases de diseño** | Nada: es el paso siguiente | `DS-00 §10`, `DOP-01` |
| 3 | `PER-H2` | Detalle de esquema de persistencia | `ARQ-01` | — | `PER-01 §8` |
...
> **Los dos primeros llevan marca por una razón distinta.** `PER-H5` es el único que **rompe un
> requisito vigente** — y por eso lleva fecha límite material, no de calendario: antes de que haya
> personas reales. `COD-01` no rompe nada; es una espera con motivo escrito, y figura aquí para que
> no se lea como un entregable que se quedó sin hacer.
