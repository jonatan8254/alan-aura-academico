...
## Qué sigue
1. **La verificación independiente del retrabajo del CDR** (`CDR-01 v1.2`, `SD-39`). **El retrabajo ya está aplicado**: los 20 hallazgos quedan resueltos —17 aplicados, `H-03` diferido con riesgo aceptado, `H-09`/`H-19` heredados por `ARQ-01`— y el propio retrabajo destapó **8 hallazgos nuevos** (`H-21`…`H-28` del acta). **Lo que falta es verificarlo, y no lo hace quien lo aplicó.**

   > **Por qué el veredicto sigue en `Reinspección requerida` aunque todo esté corregido.** Porque **aplicar no es verificar**. Quien aplicó las correcciones lleva el anclaje de haberlas justificado una por una, y firmaría su propio trabajo. Por decisión del líder, la verificación se hace en **sesión aparte**, sobre los artefactos y el acta —no sobre el razonamiento de quien corrigió—, y **con encuadre adversarial**: el encargo es *refutar* cada corrección, no confirmarla. **La limitación de fondo se declara igual:** el verificador seguirá siendo el mismo modelo, así que esto mitiga el sesgo de **recuerdo**, no alcanza la independencia que `IEEE 1028 §6.5.6.5` pide. Esa la da el equipo humano de `CLAUDE.md §2`.
   ...
   *Lo que sigue, para contexto de cómo se llegó aquí:*
   ...
   > **Lo que el CDR confirmó que está bien**, y conviene no perder de vista: 150/150 controladores con caso de prueba · los 8 conteos canónicos exactos contra los `.puml` · las 16 pantallas de `DIS-00` con clase frontera · 21/21 clases de solución declaradas y justificadas · 26/26 RF con caso de uso, secuencia y pruebas · cero patrones prematuros · cero tipos vagos. La tasa fue de **0,37 hallazgos por página** contra la base de 0,4: la revisión rindió lo esperado, ni se quedó corta ni el paquete resultó inmaduro.
...
| 15 | **Verificación independiente del retrabajo** | El retrabajo de `SD-39` está **aplicado**, no verificado. **Quien aplicó no puede firmarlo**: llevaría el anclaje de haber justificado cada corrección | **Sesión aparte**, con encuadre **adversarial** (refutar, no confirmar), sobre los artefactos y el acta. Emite `CDR-01 v1.3` | **El paso al código.** El veredicto sigue en `Reinspección requerida` hasta entonces | `CDR-01 §8-ter`, §13 |
...
> **Actualizado en SD-37.** Siguen cerrados los tres que llevaban marca: **`PER-H5`** (fila 1, `ADR-003` en
...
>
> **Quedan once abiertos.** Nueve son de diseño físico (`ARQ-01`), de construcción, de propagación documental o
> de validación del usuario, y ninguno tiene plazo material. Los **dos nuevos los abre el CDR** (filas 13 y 14),
> y la fila 13 es la única del tablero que **bloquea algo concreto: el paso al código**. Es también la única
> vez que un pendiente de esta tabla nace de una compuerta en vez de heredarse de una fase anterior — que es
> exactamente para lo que sirve una compuerta.
