...
**Propósito:** qué es cada artefacto, su ID, sus insumos y consumidores. **Fecha de creación:** 2026-07-12 · **Última actualización:** 2026-08-05 (**SD-39: el retrabajo del `CDR-01`** — los 20 hallazgos resueltos, 8 nuevos declarados, seis hechos canónicos movidos y `H-29` creado, y el veredicto **pendiente de verificación independiente**; antes SD-37: entra **`CDR-01`**, la compuerta entre el diseño detallado y el código — veredicto `Reinspección requerida`, 13 hallazgos con 2 mayores; antes SD-34: el respaldo de la base de datos queda marcado como obsoleto y se cierra el registro de `RF-24`; antes SD-33: entra `ADR-003` y **`PER-H5` queda cerrado**; antes SD-32: el modelo de clases de diseño `MC-00`/`MC-01` y `COD-01`, con el CDR como siguiente hito).
...
- **DS-00 v1.6 · DOP-01 v1.3 · TRZ-DS-01 v1.3 · DS-01…14 v1.0** (`DS-06` y `DS-09` a **v1.1** por `H-1`)**:** diagramas de secuencia (skill `uml-sequence-diagram`, modo *Generar*); **283 mensajes** (`H-22`), **150/150** controladores cubiertos, 0 flujos sin fragmento; validador **0 errores en los 14** (6 advertencias, la excepción declarada `E-2`). Capa de infraestructura **declarada no ejecutada** (`E-1` de `DS-00`). Acompañan **`DOP-01 v1.3`** (delta de **193 operaciones**, `H-23`), **`CP-00 v1.4`** (índice; los 14 archivos de casos van por v1.0, salvo `CP-01`, `CP-04`, `CP-06` y `CP-09` en **v1.1**) con **181 casos de prueba** (`H-24`) derivados de los Controladores y **`TRZ-DS-01 v1.3`** (matriz paso ↔ mensaje ↔ operación ↔ clase ↔ `CP`). SVG por generador propio, **0 colisiones**.
