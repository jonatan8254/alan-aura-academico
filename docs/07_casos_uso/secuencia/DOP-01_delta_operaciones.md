...
**ID:** DOP-01 · **Familia:** DS (secuencia, fase 2 ICONIX) · **Hogar:** `docs/07_casos_uso/secuencia/` · **Fecha:** 2026-08-01 · **Versión:** v1.5 (SD-41, `VI-02`: §6 emparejaba `borrarCompleta()` con `deshacerElBorradoIncompleto()`, operación que `H-15` retiró y que `MC-01` ya no declara; `H-23` **no se mueve** — los 193 nombres distintos ya la excluían). v1.4 (SD-40: §8 decía **192** operaciones distintas cuando `H-23` va por **193** desde `SD-39` — y este es el artefacto **dueño** del hecho, contradiciendo su propia §8 bis; se precisa además que sus **21** clases de solución son las del delta de secuencia y no las 27 de `MC-01`). v1.3 (SD-39: retrabajo del `CDR-01` — `H-20`). v1.2 (SD-32: desviarADerivacionDeCU07 cambia de receptora y §8 corrige el recuento del espacio de la solución a 21). v1.1 · **Estado:** Propuesto — cubre los **14** diagramas.
...
| `borrarCompleta()` · `dejarDeExistir()` | `CapsulaDePerfil` | problema | **Sin estados intermedios, y ahora por la razón correcta.** Hasta `SD-39` esta fila emparejaba `borrarCompleta()` con `deshacerElBorradoIncompleto()`, una reversión que `H-15` del `CDR-01` declaró **innecesaria**: el borrado alcanza a **una sola entidad**, y sobre una entidad única o la operación pasa o no pasa. No hay estado a medias del que volver. El fallo lo informa la **frontera** —`informarElFalloSinHaberSuprimidoNada()` en `DS-11:71`—, no la entidad, porque la entidad no hizo nada que deshacer. *(Fila corregida en `SD-41` por `VI-02`; `H-23` no se mueve: los 193 nombres distintos ya excluían la operación retirada.)* |
...
|---|---|---|---|
| v1.5 | 2026-08-05 | J. Sánchez | **SD-41 — `VI-02` del `CDR-01 v1.4`.** La fila de §6 emparejaba `borrarCompleta()` con `deshacerElBorradoIncompleto()` y la justificaba con «sin estados intermedios: o desaparece entera o queda como estaba». `H-15` mostró que la segunda operación protegía un **estado que no puede existir** —el borrado alcanza a una sola entidad—, y `SD-39` la retiró de `MC-01` sin actualizar esta fila. Ahora empareja `borrarCompleta()` con `dejarDeExistir()` y sitúa el informe del fallo en la **frontera**, que es donde `DS-11:71` lo dibuja. **`H-23` no se mueve:** comprobado contando sobre `MC-01`, los **193** nombres distintos ya excluían la operación retirada e incluían las dos nuevas. |
