...
| 2 | Generar cabeceras e inspeccionarlas | **NO EJECUTADA — diferida con disparador** | Decisión del líder: se generan **tras** aplicar las correcciones del CDR, y pasan **revisión humana antes de versionarse**. Es ausencia **razonada y declarada**, no silenciosa. `H-04` la bloqueaba de todos modos |
...
**Capa adicional declarada NO ejecutada:** la **revisión visual** del `.svg` de `MC-01`. El render se aplazó **a propósito** —a la espera de que el `.puml` esté firme, por si el CDR obligaba a tocarlo— y este acta demuestra que la cautela era correcta: `H-04`, `H-07`, `H-11` y `H-13` lo tocan. Se reencuadra el pendiente #12 de `ESTADO_PIPELINE`: la causa deja de ser «falta PlantUML» y pasa a ser **aplazamiento deliberado con disparador**.
