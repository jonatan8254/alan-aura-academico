...
**ID documento:** DOC-CU-11 · **Caso de uso:** CU-11 · **Alias en DCU-01:** `CU_Reinit` · **Familia:** ECU (especificación de casos de uso, fase 2 ICONIX) · **Hogar:** `docs/07_casos_uso/especificaciones/` · **Fecha:** 2026-07-31 · **Versión:** v1.2 (`CDR-01`: **`H-14`** Mayor — se **retira `RE-06`**, la auditoría del reinicio que ningún `RF` respalda; y **`H-15`** — `FE-04` deja de deshacer un borrado parcial que no puede existir sobre una entidad única). v1.1 (SD-35: `RA-03` cerrada — el borrado de la cápsula no tiene ventana de respaldo). v1.0 · **Estado:** Propuesto.
...
| Versión | v1.1 (SD-35: `RA-03` cerrada — el borrado de la cápsula no tiene ventana) |
...
|---|---|---|---|
| **v1.2** | 2026-08-04 | J. Sánchez | **`CDR-01` — retrabajo `SD-39`. Dos hallazgos.** **`H-14` (Mayor):** se **retira `RE-06`**, que exigía que «el reinicio queda fechado y auditable». **Ningún requisito lo respalda** —`RF-22` solo pide que la cápsula deje de existir, y `RF-18`, el único de auditoría, es del *kill switch* y dice «sin datos de usuario»— y ningún diagrama lo realizaba: `DS-11` no declara participante de auditoría alguno. Auditar la acción de una persona sobre **sus propios datos** persistiría un registro sobre ella que nadie pidió, contra la minimización y el canon no punitivo. **`H-15`:** `FE-04` y `RE-04` prometían deshacer un borrado parcial de la `CapsulaDePerfil`; sobre **una entidad única** ese estado intermedio **no puede existir**, así que la compensación era innecesaria —no imposible, como en `H-02`— y modelaba un fallo irreal. Ahora `FE-04` informa el fallo y la cápsula sigue intacta. Tocados: `FE-04`, el invariante de §14, `RE-04`, `RE-06` y `CA-11`. |
| v1.1 | 2026-08-04 | J. Sánchez | **SD-35: `RA-03` cerrada.** Preguntaba si el borrado de la cápsula tenía ventana de respaldo. **No la tiene:** `ADR-004-D1` fija la supresión como física e inmediata, y `ADR-003` ya había quitado los respaldos. Era la misma pregunta que `PER-H2` y que `RA-01` de `ECU-04`, registrada en tres sitios. **Ningún flujo, regla ni criterio cambia.** |
...
| **FE-04** | Borrado no realizado | Paso 3 | El Sistema no logra ejecutar el borrado de la `CapsulaDePerfil` | `500`; **informa el fallo sin haber suprimido nada** (`RE-04`) | «No pudimos reiniciar tu caracterización; inténtalo de nuevo» | La cápsula sigue completa, con `character` | **Vuelve** al paso 1 |
...
| **Invariante** | Nunca queda una `CapsulaDePerfil` **sin `character`**, estado que `RN-01.6` declara imposible: la cápsula existe entera o no existe | Inspección tras un fallo inducido (`FE-04`) |
...
| **RE-04** | **Integridad** | El borrado alcanza la `CapsulaDePerfil` **como una sola unidad**: se suprime entera o no se suprime. No hay estado intermedio que deshacer, porque **no existe** — una cápsula sin `character` es un estado que `RN-01.6` declara imposible y que este caso de uso no puede producir. **Precisado en `CDR-01` (`H-15`):** la versión anterior prometía «todo-o-nada» y diseñaba una compensación para revertir un borrado parcial; sobre una entidad única esa compensación es innecesaria, no imposible, y modelarla describía un fallo que no puede ocurrir. | Prueba de fallo inducido (`FE-04`): el Sistema informa el fallo y la cápsula sigue **intacta y completa**, sin que haya hecho falta restaurarla |
...
| RE-06 | ~~Trazabilidad~~ — **RETIRADO** | **Sin efecto desde `CDR-01` (`H-14`, Mayor). El identificador se conserva para que las citas previas resuelvan; la exigencia ya no rige.** Exigía que «el reinicio queda fechado y auditable», y **ningún requisito lo respalda**: `RF-22` solo pide que la cápsula deje de existir, y `RF-18` —el único de auditoría del proyecto— es del *kill switch* y dice expresamente «sin datos de usuario». Auditar la acción de una persona **sobre sus propios datos** persistiría un registro sobre ella que nadie pidió, contra la minimización (`PRIV-R`) y contra el canon de uso no punitivo. Esta especificación se lo había impuesto por su cuenta, y ningún diagrama lo realizaba. | — |
...
| CA-11 | Dado un fallo inducido al intentar el borrado, cuando el sistema responde, entonces **informa el fallo** y la `CapsulaDePerfil` sigue **completa, con `character`**, sin que haya hecho falta restaurarla. | FE-04 | Prueba de fallo inducido e inspección |
...
| ~~RA-03~~ ✅ **Cerrada (SD-35)** | Ambigüedad resuelta | **No hay ventana: el borrado es físico e inmediato** (`ADR-004-D1`), y además `ADR-003` quitó los respaldos. Era la misma pregunta que `PER-H2` y que `RA-01` de `ECU-04`, registrada en tres sitios. Texto original: ¿El borrado de la cápsula tiene ventana de respaldo? PER-01 fija su retención «hasta reinicio, revocación o eliminación», sin ventana declarada, mientras que la cuenta sí lleva «+ 30 días». El hallazgo PER-H2 dejó **abierto por decisión** si el hosting hace respaldos. | Define si «deja de existir» es borrado físico inmediato o purga diferida. | Se resuelve en la fase de construcción, cuando se conozca la política de respaldo del entorno. No bloquea la especificación ni el análisis de robustez. | Abierto |
