...
**ID documento:** DOC-CU-04 · **Caso de uso:** CU-04 · **Alias en DCU-01:** `CU_Elim` · **Familia:** ECU (especificación de casos de uso, fase 2 ICONIX) · **Hogar:** `docs/07_casos_uso/especificaciones/` · **Fecha:** 2026-08-01 · **Versión:** v2.1 (SD-29: precisión de `CA-01`, sin cambio de exigencia — ver historial) · **Estado:** Propuesto.
...
| Campo | Descripción |
|---|---|
| Objetivo | Que el Usuario **suprima su cuenta** y, con ella, **todos sus registros asociados**, mediante un borrado en cascada verificable y sin remanentes alcanzables desde la aplicación. |
...
| Resultado observable | La cuenta y sus registros asociados dejan de existir; las credenciales anteriores ya no dan acceso; ningún registro asociado queda recuperable desde la aplicación. |
...
| Borrado en cascada | La supresión del `Usuario` arrastra `CapsulaDePerfil`, `Consentimiento` y `ContadorDeUsoDiario` | prohibido: «borrado lógico», «marcar como eliminado» | PER-T1; el alcance exacto de la cascada es el de esa regla, ni más ni menos |
...
| 3 | Sistema | Suprime el `Usuario` y ejecuta el **borrado en cascada** de su `CapsulaDePerfil`, su `Consentimiento` y su `ContadorDeUsoDiario` | `Usuario`, `CapsulaDePerfil`, `Consentimiento`, `ContadorDeUsoDiario` | La supresión ocurre entera o no ocurre (`FE-04`); ningún registro asociado queda recuperable | — |
...
| FE-04 | Cascada interrumpida | Paso 3 | El almacenamiento falla a mitad del borrado en cascada | Deshace lo suprimido y **no** confirma la eliminación; informa que la acción no se completó | «No pudimos eliminar tu cuenta; sigue disponible e intacta» | La cuenta y sus registros quedan **completos**, nunca a medias | **Vuelve** al paso 1 |
...
| **Éxito (lo esencial)** | Ningún registro asociado a la cuenta queda recuperable desde la aplicación (RF-24, PRIV-R11) | Inspección: sin remanentes alcanzables |
...
| **Invariante** | **La eliminación es todo o nada.** Nunca queda una cuenta con parte de sus registros suprimida y parte viva | Prueba de interrupción (`FE-04`) |
...
| RE-04 | Integridad | El borrado en cascada es **atómico**: si no puede completarse, el Sistema no deja registros a medio suprimir ni confirma la eliminación (RF-24). | Prueba de interrupción: estado íntegro y sin confirmación |
...
|---|---|---|---|
| CA-01 | Dado un usuario que confirma la eliminación de su cuenta, cuando el Sistema ejecuta el borrado, entonces su `CapsulaDePerfil`, su `Consentimiento` y su `ContadorDeUsoDiario` desaparecen y ningún registro asociado queda recuperable **desde la aplicación** (mismo acotador que §14 y «Resultado observable»; el respaldo de infraestructura es excepción declarada — `RA-01`, `PER-H5`). | Flujo básico | Inspección del almacenamiento **alcanzable desde la aplicación** |
...
| CA-10 | Dada una interrupción a mitad del borrado en cascada, cuando el usuario vuelve a entrar, entonces encuentra su cuenta **completa** —sin registros a medio suprimir— y el Sistema no le confirmó una eliminación que no ocurrió. | FE-04 | Prueba de interrupción |
...
|---|---|---|---|---|---|
| RA-01 | Ambigüedad | RF-24 exige que «no quede dato asociado recuperable», pero plan §4.14 fija «hasta eliminación o cierre **+ 30 días**». Es la misma cuestión que PER-01 registra como **PER-H2**. | Define si la supresión es física e inmediata o lógica con purga diferida, y por tanto si el esquema necesita marca de baja | El borrado es inmediato **de cara al Usuario**: nada asociado queda alcanzable desde la aplicación. La ventana de +30 días, si aplica, es retención de respaldo por entorno. PER-01 §8 lo deja **abierto por decisión**, a resolver en construcción con el *hosting* a la vista. **Actualización (SD-29):** `ADR-002-D6` fijó ese respaldo en S3 versionado, lo que convirtió la ambigüedad genérica en un hallazgo concreto — **`PER-H5`** — porque ninguna regla vigente cubre el borrado del respaldo. Sigue siendo la misma pregunta; ahora tiene arquitectura y nombre propio. | **Abierto por decisión** |
...
| RA-04 | Decisión de esta especificación | La **atomicidad** del borrado en cascada (`RE-04`, `FE-04`) no estaba exigida en ningún artefacto previo. | Sin ella, una interrupción dejaría una cuenta a medio suprimir, que contradice RF-24 y PRIV-R11 | **Decidido aquí:** todo o nada. Se declara como invariante de §14 y como `RE-04` para que la construcción lo trate como requisito y no como detalle. | **Decidido** |
...
| 13 | Postcondiciones verificables | ⚠️ | Verificables por inspección, **salvo el alcance temporal de «sin remanentes»**: mientras `PER-H2` siga abierta no puede probarse si la supresión es física o diferida (`RA-01`) |
| 14 | Sin detalle de implementación | ✅ | Caja negra; la atomicidad se enuncia como efecto observable, no como mecanismo |
