...
**Naturaleza:** registro de decisiones de arquitectura. **Supera:** ADR-001-D1, ADR-001-D2 y ADR-001-D5. **Acotada por `ADR-003` (SD-33):** `D6` deja de cubrir los respaldos del almacén operativo. **Deja intactas:** ADR-001-D3, D4, D6 y D7.
...
> ⚠️ **ACOTADA por `ADR-003` (SD-33), no superada.** De las tres cosas que esta decisión guardaba —configuración, activos y **respaldos**— la tercera **desaparece**: el almacén operativo ya no se respalda. Motivo: era el único contenido de S3 que es dato personal, y por tanto la causa de `PER-H5`. La configuración y los activos siguen aquí, con su versionado intacto, que es la razón por la que se eligió S3. Léase el resto de esta decisión con esa resta hecha.
...
| 5 | ~~El respaldo en S3 escapa al borrado en cascada~~ — **`PER-H5` CERRADO en `ADR-003`** (SD-33): el almacén operativo no se respalda. En su lugar queda su precio, que es operativo y no de canon: **la pérdida de ese almacén es irrecuperable** (`R-11` reformulado). | ✅ cerrado; el riesgo residual, asumido |
...
|---|---|---|---|
| v1.1 | 2026-08-04 | J. Sánchez | **SD-33: `D6` acotada por `ADR-003`.** De las tres cosas que guardaba —configuración, activos y **respaldos**— desaparece la tercera: el almacén operativo deja de respaldarse, que es lo que cierra `PER-H5`. Su advertencia `[I2]` sobre los respaldos se marca resuelta y se conserva por trinquete. §3 fila 5 pasa de consecuencia abierta a **cerrada**. **D1…D5, D7 y D8 intactas**; el versionado de configuración y activos, intacto. |
