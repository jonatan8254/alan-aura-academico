...
**ID:** PER-01 · **Familia:** requisitos de datos · **Hogar:** `docs/03_requisitos/` · **Fecha:** 2026-08-01 · **Versión:** v1.7 (`SD-50`: `PER-H4` **CERRADO** por `ARQ-01-D2` — campos y llave de `ContadorDeUsoDiario` fijados; con él, los cuatro hallazgos de este documento quedan cerrados). v1.6 (SD-42: se repone la fila `v1.5` que faltaba en el historial y se reordena — defecto `H-25`, hallado por el **bloque 6** del verificador al escribirlo). v1.5 (SD-35: `PER-H2` CERRADO; valor inicial del kill switch). v1.4 (SD-33: `PER-H5` CERRADO por `ADR-003` — el almacén operativo no se respalda, `PER-T1` vuelve a ser completa). v1.3 (SD-30: granularidad de `OperationalEvent` fijada **por llamada**, y su §3.6 deja de reclamar las cuatro cifras de CU-09) · **Estado:** Propuesto.
...
|---|---|---|---|
| v1.7 | 2026-08-05 | S. Bedoya | **SD-50: `PER-H4` CERRADO** por `ARQ-01-D2`. Campos y llave de `ContadorDeUsoDiario` fijados: un ítem por titular y por día (`SK=CONTADOR#<fecha>`), con `contador`, `fecha` y `ttl` a 30 días, en la tabla `Titular` de diseño de tabla única. Con este cierre, **los cuatro hallazgos de `PER-01` quedan cerrados** (`PER-H1`/`PER-H3` en SD-26, `PER-H5` en SD-33, `PER-H2` en SD-35, `PER-H4` aquí). §3.5, §8 y §11 actualizados. |
| v1.6 | 2026-08-05 | J. Sánchez | **SD-42 — reparación del registro, no del contenido.** La ficha declaraba `v1.5` desde `SD-35` y el historial **nunca registró esa versión**; además las filas iban desordenadas (v1.3, v1.4, v1.2, v1.1, v1.0). Es el defecto `H-25`, que en este repositorio alcanzó a **ocho** artefactos. **Lo encontró el bloque 6 de `verificar_coherencia.py` mientras se escribía**, que es el argumento más fuerte a favor de que ese bloque exista: se había reportado y corregido a mano seis veces sin que nada impidiera la séptima. **Ninguna afirmación del mapa de persistencia cambia.** |
| v1.5 | 2026-08-04 | J. Sánchez | **SD-35: `PER-H2` CERRADO** por `ADR-004-D1`. La supresión de cuenta es **física e inmediata**, sin ventana de gracia ni marca de baja: la ventana de «+30 días» del plan §4.14 era una ambigüedad sintáctica de la fuente, no un hueco de diseño, y el MVP no tiene «cierre de cuenta». Con `PER-H5` (SD-33) y este, **`RF-24` pasa a cumplirse** según el diseño. Se fija además el **valor inicial del *kill switch*** (`ADR-004-D2`: arranca habilitado). *(Fila repuesta en `SD-42`: la versión existía en la ficha desde SD-35 y el historial nunca la registró — el defecto `H-25`, que en este repositorio ya alcanzó a ocho artefactos.)* |
| v1.4 | 2026-08-04 | J. Sánchez | **SD-33: `PER-H5` CERRADO** por `ADR-003`. El almacén operativo **no se respalda**, así que la fila «S3 — respaldos» de §1.0 desaparece y **`PER-T1` vuelve a ser completa**: no hay copia que escape a la cascada. Se cerró **quitando el segundo almacén**, no acotándolo, y **antes de `ARQ-01`** porque resultó ser un no-objetivo declarado y no diseño físico. **Precio declarado:** perder el almacén operativo es irrecuperable (`R-11` reformulado a operativo). **Quedan abiertos `PER-H2` y `PER-H4`**; el primero es ahora lo único que impide cumplir `RF-24` de forma inmediata. |
...
| ~~**S3 — respaldos**~~ | ⛔ **NO EXISTEN, desde `ADR-003` (SD-33).** El almacén operativo **no se respalda**: sin exportaciones, sin recuperación a un punto en el tiempo, sin copia de su contenido fuera de él. Era el único sitio de S3 con dato personal, y quitarlo es lo que **cierra `PER-H5`**. El versionado se queda solo donde no hay dato personal (`ADR-003-D2`). **Precio declarado:** un fallo del almacén es irrecuperable — riesgo `R-11` reformulado. | `ADR-003-D1`/`D2`, PER-T1, PRIV-R11 |
...
|---|---|---|---|---|---|
| 1 | `User` | Cuenta, rol, declaración de adultez | CU-02 «Registrar cuenta» | **No** | **Hasta la eliminación, y en ese momento** — supresión física e inmediata, sin ventana de gracia (`ADR-004-D1`, SD-35). Los «+ 30 días» del plan §4.14 correspondían al **cierre** de cuenta, función que el MVP no tiene [E1] |
...
**Valor inicial: `habilitado`** (`ADR-004-D2`, SD-35). Un entorno recién aprovisionado, o uno que pierda esta entidad, arranca con el chatbot activo. Cierra `PRE-03` de `ECU-10`, que declaraba que ningún artefacto lo fijaba. **No es *fail-closed*, y se declara:** el motivo es la conveniencia de la demostración académica, y la condición de reversa es material — **en cuanto haya personas reales, pasa a `deshabilitado`**. El *gate* de seguridad y el *fallback* no dependen de este estado (`SEG-R1`, `RNF-06`). [E1]
...
Campos concretos y granularidad: ~~no especificados en ningún artefacto~~ **cerrado en `ARQ-01-D2`** (`SD-50`): un ítem por titular y por día (`SK=CONTADOR#<fecha>`) con `contador`, `fecha` y `ttl` a 30 días. Ver **PER-H4**, cerrado.
...
| ID | Regla | Traza |
|---|---|---|
| **PER-T1** | **Borrado en cascada:** eliminar `User` suprime `ConsentRecord` + `InitialConversationProfile` + `DailyUsageCounter` de ese usuario. **Desde `ADR-003` la regla vuelve a ser completa**: el hueco que `ADR-002-D6` había abierto —los respaldos en S3 y las versiones anteriores— desaparece porque **el almacén operativo deja de respaldarse**. `PER-H5` cerrado. Lo que sigue abierto es la **ventana de retención** de `PER-H2`, que es otra cosa. | PRIV-R11, RN-04.4, RF-24, **primer criterio de aceptación de ECU-04 v2.0** —se cita en prosa porque la renumeración del PDR-01 cambió los identificadores— [E1] |
...
| ~~**PER-H2**~~ | ✅ **CERRADO (SD-35, `ADR-004-D1`)** | RF-24 exige que tras eliminar la cuenta «no quede dato asociado recuperable», pero plan §4.14 fija «hasta eliminación o cierre **+ 30 días**». Ya registrada como RA-01 en ECU-04 §21. | Definía si la eliminación es borrado físico inmediato o lógico con purga diferida — y por tanto si el esquema necesita marca de baja. **Respuesta: físico e inmediato, sin marca de baja.** | ✅ **CERRADO en SD-35.** Resultó ser una **ambigüedad sintáctica de la fuente**, no un hueco de diseño: «Hasta eliminación o cierre + 30 días» admite dos análisis, y este hallazgo asumía —sin decirlo— el que pone al plan en contradicción con `RF-24`. **El MVP no tiene «cierre de cuenta»**, verificado sobre los 26 RF de `REQ-01`, así que esa ventana se refería a un escenario que aquí no existe. Argumento completo y marca `[I2]` en `ADR-004-D1`. Cierra a la vez `RA-01` de `ECU-04` y `RA-03` de `ECU-11`, que eran la misma pregunta registrada en tres sitios. |
...
| ~~**PER-H4**~~ | Hueco menor | `DailyUsageCounter` está nombrada y acotada (por usuario, diaria, ≤30 días) pero **sus campos y su llave no están especificados** en ningún artefacto. | `ARQ-01-D2` (`SD-50`, 2026-08-05): campos y llave fijados — `SK=CONTADOR#<fecha>` bajo el titular, con `contador`, `fecha`, `ttl`. | ✅ **CERRADO en `ARQ-01-D2`.** |
| ~~**PER-H5**~~ | ✅ **CERRADO (SD-33, `ADR-003`)** | `ADR-002-D6` crea un **segundo lugar donde vive el dato personal**: el respaldo exportado en S3. Ninguna regla vigente lo alcanza. `PER-T1` y `PRIV-R11` enumeran el borrado en cascada sobre `ConsentRecord`, `InitialConversationProfile` y `DailyUsageCounter` — escritas cuando no había respaldos—. Y el **versionado**, que `ADR-002-D6` adopta por sus ventajas sobre los textos legales, significa que **borrar un objeto no lo borra**: sobreviven las versiones anteriores. | Define la retención y el borrado de los respaldos, y si el versionado se aplica al mismo contenedor que los datos personales o solo al de configuración. Sin eso, un usuario puede ejercer `RF-24` («no queda dato asociado recuperable») y sus datos **seguir existiendo** en el respaldo. | ✅ **CERRADO en SD-33** por `ADR-003`, y **antes de `ARQ-01`**, no en él: la decisión resultó no ser de diseño físico sino un **no-objetivo declarado** —«el MVP no respalda el almacén de datos personales»—, de la misma clase que «no cachear respuestas del LLM». Se cerró **quitando el segundo almacén**, no acotándolo: sin respaldos no hay copia que escape a la cascada. Las alternativas acotadas —ventana de expiración, cifrado por usuario— quedan preparadas en `ADR-003 §3` por si se dispara la condición de reversa. **Precio declarado:** la pérdida del almacén operativo es irrecuperable (`R-11` reformulado). |
...
- **Hallazgos: los cuatro cerrados.** `PER-H5` cerró en SD-33 (`ADR-003`: el almacén operativo no se respalda), **`PER-H2` en SD-35** (`ADR-004`: la supresión es física e inmediata) y **`PER-H4` en SD-50** (`ARQ-01-D2`: campos y llave de `ContadorDeUsoDiario` fijados). Con `PER-H5` y `PER-H2` cerradas, **`RF-24` pasa a cumplirse** según el diseño. Los tres eran diseño de esquema y su sitio era `ARQ-01`, que ya los resolvió; ninguno bloqueó los diagramas de secuencia mientras estuvieron abiertos.
