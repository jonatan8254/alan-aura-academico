...
| RF-24 | **Eliminar la cuenta** con **borrado en cascada**. | RN-04.4 | Eliminada la cuenta, **no queda dato asociado recuperable**. **Sin excepciones abiertas desde SD-35:** `PER-H5` cerrado en `ADR-003` (el almacén operativo no se respalda) y `PER-H2` cerrado en `ADR-004` (la supresión es **física e inmediata**, sin ventana de gracia ni marca de baja). El `OperationalEvent` que sobrevive no es dato asociado: es irreidentificable por construcción (`CA-11` de `ECU-04`, `PER-T2`). **Cumplido según el diseño; su verificación empírica corresponde a la fase 4.** |
