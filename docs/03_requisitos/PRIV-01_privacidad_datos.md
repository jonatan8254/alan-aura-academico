...
**ID:** PRIV-01 · **Hogar:** `docs/03_requisitos/` · **Fecha:** 2026-07-25 · **Versión:** v1.8 (`SD-50`: `§4.1` deja de estar condicionado — `ARQ-01-D1` fija Vercel como intermediario de la API, así que la fila de encargado del tratamiento pasa de «No — condicionado» a «Sí»). v1.7 (SD-35: la cascada es además inmediata). v1.6 (SD-33: `PRIV-R11` recupera la cascada completa; se retira el acotador «alcanzables desde la aplicación»). v1.5 (SD-29: encargados del tratamiento en §4.1 —tres partes donde había dos—, **región de AWS como pregunta abierta** bajo la Ley 1581, y sesión en cookie firmada. **`PRIV-R11` gana el acotador «alcanzables desde la aplicación»** (mismo texto que `ECU-04 §14`): no cambia qué se exige, precisa dónde se verifica, tras `PER-H5`. El resto, PRIV-R1…R10 y R12, sin cambios. SD-26: la cápsula **siempre existe** tras el onboarding con `character` como mínimo — PER-H1 resuelta; `estado` del directorio acotado en PRIV-R10 — PER-H3 resuelta; SD-22:  cápsula = `ContextoInicialConversacionalV1` de 5 campos + 2 metadatos en el inventario §2 y PRIV-R1; PRIV-R9 intacta; SD-15: cuenta, contadores operativos y qué NO ve el admin; SD-17: inventario mapeado 1:1 a las 7 entidades exactas del plan).
...
| PRIV-R3.1 | **Qué significa «granular», precisado en el PDR-01.** El `Consentimiento` tiene **dos capas independientes**. La **capa base** autoriza procesar lo mínimo para conversar —la edad declarada, `character` y el turno en curso—: sin ella no hay conversación. La **capa de personalización** autoriza que los **cuatro autorreportes** de la cápsula orienten la conversación: sin ella se conversa igual, con acompañamiento genérico. Revocar la personalización (CU-12) **no es punitivo**: el Usuario conserva el servicio. | RN-07, RN-01.5, RN-01.6 |
| PRIV-R3.2 | El descarte que exige `PRIV-R3` alcanza a los **cuatro autorreportes**, no a `character`: por `RN-01.6` es precondición funcional del chat y lo cubre la capa base. Borrar la cápsula entera, `character` incluido, es **CU-11** y tiene efecto distinto: deja al Usuario sin poder conversar. | RN-01.6, RN-04.3 |
...
| PRIV-R11 | La **eliminación de cuenta** borra en **cascada** todos los datos asociados (cápsula, consentimiento, contadores del usuario). **Desde `ADR-003` la cascada es completa**: el almacén operativo **no se respalda**, así que no hay copia fuera de su alcance — el acotador «alcanzables desde la aplicación» deja de hacer falta y se retira. Y **desde `ADR-004` (SD-35) es también inmediata**: supresión física, sin ventana de gracia ni marca de baja — `PER-H2` cerrado. | RN-04.4 |
...
|---|---|---|
| **Vercel** | Sirve la interfaz **e intermedia la API** (reescrituras) | **Sí.** `ARQ-01-D1` (2026-08-05) fijó Vercel como intermediario de las peticiones para resolver el origen cruzado sin dominio propio: **todo el tráfico —credenciales y cápsula incluidas— pasa por su infraestructura** antes de llegar a API Gateway. Deja de estar condicionado. |
