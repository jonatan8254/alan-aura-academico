---
source_file: "PMBOK_Edicion_7.md"
type: "concept"
community: "PMBOK Earned Value Measurement"
tags:
  - graphify/concept
  - graphify/EXTRACTED
  - community/PMBOK_Earned_Value_Measurement
source_path: "libros_y_normas_markdown/PMBOK_Edicion_7.md"
source_absolute_path: "C:\GitHub\SmartAID\libros_y_normas_markdown\PMBOK_Edicion_7.md"
source_note: "[[02_Fuentes/Conocimiento/PMBOK_Edicion_7]]"
---

# Dominio de Desempeño de la Medición

## Origen
- Fuente en este vault: [[02_Fuentes/Conocimiento/PMBOK_Edicion_7]]
- Original canonico (proyecto principal, solo lectura): `libros_y_normas_markdown/PMBOK_Edicion_7.md`


## Connections
- [[8 Dominios de Desempeño del Proyecto]] - `references` [EXTRACTED]
- [[Análisis del Valor Ganado (EVA)]] - `references` [INFERRED]
- [[Dominios de Desempeño del Proyecto]] - `references` [EXTRACTED]
- [[Gestión del Valor Ganado]] - `references` [EXTRACTED]
- [[Indicadores Clave de Desempeño (KPI)]] - `references` [EXTRACTED]
- [[Línea Base]] - `references` [EXTRACTED]
- [[Métrica]] - `references` [EXTRACTED]
- [[Métricas]] - `references` [EXTRACTED]
- [[Tablero de Control]] - `references` [EXTRACTED]

#graphify/concept #graphify/EXTRACTED #community/PMBOK_Earned_Value_Measurement